...
class nordictap_transmitter(gr.sync_block):
    ...
    def __init__(self, channel_map, address, payload, channel_index, sequence_number, big_packet, data_rate, ack, crc_length):
        gr.sync_block.__init__(
            self, name="Nordictap Transmitter", in_sig=None, out_sig=None)

        self.channel_map = channel_map #[channel_map]
    ...
    def transmit(self, msg):	
        ...
        nordictap = [self.channel_index] + [
            channel, self.data_rate, len(self.address), len(self.payload), self.sequence_number, self.ack, self.crc_length, self.big_packet]
